...
import { useRoqClient, useUserRoleFindFirst } from 'lib/roq';
...
function UserRoleEditPage() {
  ...
  const { data, error, isLoading, mutate } = useUserRoleFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: UserRoleInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.user_role.update({
        data: values as RoqTypes.user_role,
        where: {
          id,
        },
      });
