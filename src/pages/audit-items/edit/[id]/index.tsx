...
import { useRoqClient, useAuditItemFindFirst } from 'lib/roq';
...
function AuditItemEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAuditItemFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AuditItemInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.audit_item.update({
        data: values as RoqTypes.audit_item,
        where: {
          id,
        },
      });
