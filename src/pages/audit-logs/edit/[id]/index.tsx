...
import { useRoqClient, useAuditLogFindFirst } from 'lib/roq';
...
function AuditLogEditPage() {
  ...
  const { data, error, isLoading, mutate } = useAuditLogFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: AuditLogInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.audit_log.update({
        data: values as RoqTypes.audit_log,
        where: {
          id,
        },
      });
