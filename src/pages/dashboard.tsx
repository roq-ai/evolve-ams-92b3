...
import { useRoqPlatformUserProfiles, useAuditTypeCount, useAuditLogCount, useAuditItemCount } from 'lib/roq';

import { ManagerListPage as ShowCaseList } from 'pages/managers';

function DashboardPage() {
  const { session } = useSession();

  const auditTypeStat = useAuditTypeCount();

  const auditLogStat = useAuditLogCount();

  const auditItemStat = useAuditItemCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Audit Types',
      result: auditTypeStat?.data,
      isLoading: auditTypeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Audit Logs',
      result: auditLogStat?.data,
      isLoading: auditLogStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Audit Items',
      result: auditItemStat?.data,
      isLoading: auditItemStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`managers`} passHref legacyBehavior>
                  <Button
