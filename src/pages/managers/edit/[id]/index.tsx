...
import { useRoqClient, useManagerFindFirst } from 'lib/roq';
...
function ManagerEditPage() {
  ...
  const { data, error, isLoading, mutate } = useManagerFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: ManagerInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.manager.update({
        data: values as RoqTypes.manager,
        where: {
          id,
        },
      });
    ...
    >
      <Box rounded="md">
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <TextInput
            error={formik.errors.manager_name}
            label={'Manager Name'}
            props={{
              name: 'manager_name',
          ...
          <TextInput
            error={formik.errors.manager_phone}
            label={'Manager Phone'}
            props={{
              name: 'manager_phone',
          ...
          <TextInput
            error={formik.errors.manager_address}
            label={'Manager Address'}
            props={{
              name: 'manager_address',
              placeholder: 'Manager Address',
              value: formik.values?.manager_address,
              onChange: formik.handleChange,
            }}
          />
