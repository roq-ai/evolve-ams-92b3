...
import { useAuditFindFirst, useRoqClient } from 'lib/roq';

import { AuditInterface } from 'interfaces/audit';
import { AuditItemListPage } from 'pages/audit-items';
...
function AuditViewPage() {
  ...
  const { data, error, isLoading } = useAuditFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('audit', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/audits/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  label="Audit Date"
                  text={data?.audit_date ? format(data?.audit_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Audit Summary" text={data?.audit_summary} />

                <FormListItem label="Audit Audit Result" text={data?.audit_audit_result} />
