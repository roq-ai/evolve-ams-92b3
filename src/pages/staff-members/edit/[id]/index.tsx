...
import { useRoqClient, useStaffMemberFindFirst } from 'lib/roq';
...
function StaffMemberEditPage() {
  ...
  const { data, error, isLoading, mutate } = useStaffMemberFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: StaffMemberInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.staff_member.update({
        data: values as RoqTypes.staff_member,
        where: {
          id,
        },
      });
